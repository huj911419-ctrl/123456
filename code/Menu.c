...
int16 ra_hard_outer = 1850;    /* 直角弯HARD阶段外侧电机占空比，外侧轮推动车身转向 */
int16 ra_hard_yaw = 39;        /* 直角弯HARD阶段退出航向角阈值（度），IMU累计转过此角度退出 */
int16 ra_slow_row = 28;        /* 直角弯SLOW阶段触发行号：IP最大行>=此值时进入减速 */
int16 ra_slow_pct = 36;        /* 直角弯SLOW阶段速度百分比（30%=更快刹住） */
int16 ra_turn_row = 48;        /* 直角弯APPROACH阶段触发行号：IP最大行>=此值时准备转弯 */
