...
int16 motor_speed = 220;       /* 目标电机速度（PWM占空比单位），范围0~600，步进20 */
int16 motor_enable = 0;       /* 电机使能开关：0=禁用（停止），1=启用（允许运行） */
int16 motor_run_time =16;     /* 电机最大运行时间（秒） */
...
int16 cam_exposure = 300;      /* 摄像头曝光时间（行周期数），值越大画面越亮 */
...
int16 ra_hard_outer = 1950;    /* 直角弯HARD阶段外侧电机占空比，外侧轮推动车身转向 */
int16 ra_hard_yaw =45;        /* 直角弯HARD阶段退出航向角阈值（度），IMU累计转过此角度退出 */
int16 ra_slow_row = 28;        /* 直角弯SLOW阶段触发行号：IP最大行>=此值时进入减速 */
int16 ra_slow_pct = 20;        /* 直角弯SLOW阶段速度百分比（20%=更快刹住） */
int16 ra_turn_row = 46;        /* 直角弯APPROACH阶段触发行号：IP最大行>=此值时准备转弯 */
int16 ra_approach_frames = 3;  /* 直角弯APPROACH阶段刹车稳车帧数，结束后进入HARD急转 */
int16 ra_post_turn_gap_frames = 2; /* 上一个弯结束后，延迟几帧才允许下一次RA启动 */
...
static MenuItem items_main[] = {
    ...
    {"Speed", &motor_speed, 0, 400, 20},      /* 速度设定：0~600，步进20 */
};
