...
/* 行扫描过程中首次丢失行的行号，-1 表示未丢失。用于路口/弯道检测 */
...
/* 中点列号环形缓冲区，存储最近若干帧的有效中点列号 */
static int16 s_center_buf[IP_COL_BUF_SIZE];

/* 中点缓冲区已存元素计数（不超过 IP_COL_BUF_SIZE） */
...
/**
 * @brief 将 int16 阈值值钳位到 [20, 240] 范围并转为 uint16
 *
...
{
    /* 若值低于下限20，强制设为20，防止阈值过低导致二值化全白 */
    if (value < 20) value = 20;
    /* 若值高于上限240，强制设为240，防止阈值过高导致二值化全黑 */
 ...
 * 压缩后图像存入 image_0[COMP_H][COMP_W]，后续所有处理基于此压缩图像。
 *
 * 算法：对原始图像每2行取1行、每2列取1列，等价于 nearest-neighbor 缩放。
 * 原始 188x120 -> 压缩后 94x60，像素数减少到 1/4，显著降低后续处理计算量。
 */
static void compress_image(void)
{
    /* 外层循环：遍历压缩后图像的每一行（0 到 COMP_H-1，即 0~59） */
    for (uint8 i = 0u; i < COMP_H; i++)
    {
        /* 目标行指针，指向压缩后图像第 i 行的起始位置 */
        uint8 *dst = image_0[i];
        /* 源行指针，指向原始图像第 i*2 行（步长为2，隔行取样） */
        const uint8 *src = mt9v03x_image[(uint16)i * 2u];

        /* 内层循环：遍历压缩后图像的每一列（0 到 COMP_W-1，即 0~93） */
        for (uint8 j = 0u; j < COMP_W; j++)
        {
            /* 隔列抽样，取原始图像第 j*2 列的像素值赋给压缩图像 */
            *dst++ = *src;
            src += 2u;
        }
    }
...
/* ========================================================================
 * Otsu 自适应阈值算法（基于压缩后 94x60 图像）
 * ======================================================================== */

/**
 * @brief Otsu 自适应阈值算法（基于压缩后 94x60 图像）
 *
 * Otsu 算法（大津法）通过最大化类间方差自动选取最佳二值化阈值。
 * 实现步骤：
 *   1. 统计灰度直方图 s_hist[256]
 *   2. 遍历所有可能阈值 t (0~255)，计算前景/背景的类间方差
 *   3. 选取使类间方差最大的 t 作为阈值
 *   4. 阈值不低于 TF_OTSU_MIN_THRESHOLD（75），防止低对比度场景阈值过低
 *   5. 加上菜单可调的 threshold_bias 偏移量
 *
 * @return 计算得到的二值化阈值（uint16），范围 [20, 240]
 */
static uint16 calc_otsu(void)
{
    /* 图像总像素数 = 94 * 60 = 5640，用于计算前景/背景的权重比例 */
    uint32 total = (uint32)COMP_H * COMP_W;
    /* 所有像素灰度值的加权总和，用于快速计算前景均值 */
    uint32 sum = 0u;
    /* 默认阈值（最低保护值），当 Otsu 计算结果过低时使用此值 */
    uint16 threshold = (uint16)TF_OTSU_MIN_THRESHOLD;

    /* ---- 步骤1：清零直方图并统计各灰度级出现次数 ---- */
    /* 将 256 个灰度级的计数全部清零 */
    for (uint16 i = 0; i < 256; i++) s_hist[i] = 0;
    /* 遍历压缩图像的每个像素，累加对应灰度级的直方图计数 */
    for (uint8 i = 0; i < COMP_H; i++)
        for (uint8 j = 0; j < COMP_W; j++)
            s_hist[image_0[i][j]]++;

    /* ---- 步骤2：计算总灰度加权和 sum = sigma(t * hist[t]) ---- */
    /* 遍历所有灰度级，计算加权总和，用于后续快速求前景均值 */
    for (uint16 t = 0; t < 256; t++)
        sum += (uint32)t * s_hist[t];

    /* 背景类（灰度值 <= t）的灰度加权和，逐步累加 */
    uint32 sumB = 0u;
    /* 背景类的像素数（权重），逐步累加 */
    uint32 wB = 0u;
    /* 记录遍历过程中遇到的最大类间方差得分 */
...
static void denoise_binary_image(void)
{
    ...
    for (uint8 i = 0u; i < COMP_H; i++)
    {
        ...
        {
            uint8 white_cnt =
                (uint8)((prev[j - 1u] == Image_WHITE) +
                        (prev[j]      == Image_WHITE) +
                        (prev[j + 1u] == Image_WHITE) +
                        (src[j - 1u]  == Image_WHITE) +
                        (src[j]       == Image_WHITE) +
                        (src[j + 1u]  == Image_WHITE) +
                        (next[j - 1u] == Image_WHITE) +
                        (next[j]      == Image_WHITE) +
                        (next[j + 1u] == Image_WHITE));

            /* 判断当前像素是白色还是黑色，分别应用不同的降噪规则 */
            if (src[j] == Image_WHITE)
            {
                /* 当前为白色：需至少 TF_DENOISE_WHITE_MIN(4) 个白色邻居才保留，否则判定为白噪点变黑 */
                dst[j] = (white_cnt >= TF_DENOISE_WHITE_MIN) ? Image_WHITE : Image_BLACK;
            }
            else
            {
                /* 当前为黑色：需至少 TF_DENOISE_BLACK_FILL(7) 个白色邻居才翻白（填补黑噪点），否则保留黑色 */
...
static inline uint8 is_right_edge(int16 row, int16 col)
{
    ...
    if (row < 0 || row >= TF_IMG_H || col < 1 || col >= TF_IMG_W - 1) return 0u;
    const uint8 *line = Image_Binarize[row];
    /* 右边缘条件：col-1为白 且 col为白 且 col+1为黑（白->黑跳变） */
    return (line[col - 1] == Image_WHITE &&
            line[col]     == Image_WHITE &&
            line[col + 1] != Image_WHITE) ? 1u : 0u;
}
...
/**
 * @brief 从 start 向右扫描寻找左边缘（黑->白跳变点）
 *
 * 在指定行中，从 start 列向 end 列逐列扫描，找到第一个左边缘即返回。
 * 左边缘 = 左边黑、当前白、右边白。
 *
 * @param row   行号
 * @param start 起始列号（含）
 * @param end   结束列号（含）
 * @return 找到的边缘列号，未找到返回 TF_INVALID(-1)
 */
static int16 scan_left_edge_right(int16 row, int16 start, int16 end)
{
    /* 将起始列钳位到 >= 1（左边缘检测需要访问 col-1） */
    if (start < 1) start = 1;
    /* 将结束列钳位到 <= TF_IMG_W-2（需要访问 col+1） */
    if (end >= TF_IMG_W - 1) end = TF_IMG_W - 2;
    /* 若起始列大于结束列，搜索范围无效，直接返回 */
...
/**
 * @brief 从 start 向左扫描寻找左边缘（黑->白跳变点）
 *
 * 在指定行中，从 start 列向 end 列逐列反向扫描，找到第一个左边缘即返回。
 * 用于从中心向左搜索赛道左边界。
 *
 * @param row   行号
 * @param start 起始列号（含），扫描方向为从右向左
 * @param end   结束列号（含）
 * @return 找到的边缘列号，未找到返回 TF_INVALID(-1)
 */
static int16 scan_left_edge_left(int16 row, int16 start, int16 end)
{
    /* 将起始列钳位到 <= TF_IMG_W-2（右边界保护） */
    if (start >= TF_IMG_W - 1) start = TF_IMG_W - 2;
    /* 将结束列钳位到 >= 1（左边缘检测需要访问 col-1） */
    if (end < 1) end = 1;
...
/**
 * @brief 从 start 向左扫描寻找右边缘（白->黑跳变点）
 *
 * 在指定行中，从 start 列向 end 列逐列反向扫描，找到第一个右边缘即返回。
 * 右边缘 = 左边白、当前白、右边黑。
 *
 * @param row   行号
 * @param start 起始列号（含），扫描方向为从右向左
...
{
    /* 将起始列钳位到 <= TF_IMG_W-2（右边缘检测需要访问 col+1） */
    if (start >= TF_IMG_W - 1) start = TF_IMG_W - 2;
    /* 将结束列钳位到 >= 1 */
    if (end < 1) end = 1;
    /* 若起始列小于结束列（反向范围无效），直接返回 */
...
/**
 * @brief 从 start 向右扫描寻找右边缘（白->黑跳变点）
 *
 * 在指定行中，从 start 列向 end 列逐列扫描，找到第一个右边缘即返回。
 * 用于从中心向右搜索赛道右边界。
 *
 * @param row   行号
 * @param start 起始列号（含）
...
static int16 scan_right_edge_right(int16 row, int16 start, int16 end)
{
    /* 将起始列钳位到 >= 1 */
    if (start < 1) start = 1;
    /* 将结束列钳位到 <= TF_IMG_W-2（右边缘检测需要访问 col+1） */
    if (end >= TF_IMG_W - 1) end = TF_IMG_W - 2;
    /* 若起始列大于结束列，搜索范围无效，直接返回 */
    if (start > end) return TF_INVALID;
...
/* 前向声明：将边缘列号钳位到合法范围 [1, TF_IMG_W-2] */
...
/**
 * @brief 在指定行中，以 ref_col 为中心查找最近的白色段（赛道区域）
 *
 * 搜索策略：
 *   1. 先检查 ref_col 处是否为白色
 *   2. 若不是，向两侧交替扩展搜索最近的白色像素作为种子点
 *   3. 找到种子后向左右扩展得到完整的白色段 [left, right]
 *   4. 对白色段宽度进行合法性检查：
 *      - 过宽（> TF_MAX_TRACK_WIDTH）：可能是噪声区域，返回失败
 *      - 过窄（< TF_MIN_TRACK_WIDTH）：使用半宽度补全
 *
 * @param row      行号
 * @param ref_col  参考列号（搜索起点）
 * @param out_lb   输出参数：白色段左边界列号
 * @param out_rb   输出参数：白色段右边界列号
 * @return 1=找到有效白色段，0=未找到
 */
                                         ...
                                         int16 *out_lb, int16 *out_rb)
{
    ...
    /* 将参考列钳位到合法范围 [1, TF_IMG_W-2]，避免越界访问 */
 ...
 *
 * 边缘列不能为 0 或 TF_IMG_W-1，因为 is_left_edge/is_right_edge 需要访问相邻列（col-1 和 col+1）。
 * 钳位到 [1, TF_IMG_W-2] 保证所有边缘检测函数不会越界。
 *
 * @param col 输入列号
 * @return 钳位后的列号，范围 [1, TF_IMG_W-2]
 */
static int16 clamp_edge_col(int16 col)
{
    /* 下限保护：列号不能小于1 */
    if (col < 1) return 1;
    /* 上限保护：列号不能大于 TF_IMG_W-2 */
 ...
 *
 * 中线列号可以取图像最边缘的列（0 或 TF_IMG_W-1），
 * 不像边缘列那样需要访问相邻列。
 *
 * @param col 输入列号
 * @return 钳位后的列号，范围 [0, TF_IMG_W-1]
 */
static int16 clamp_center_col(int16 col)
{
    /* 下限保护：列号不能小于0 */
    if (col < 0) return 0;
    /* 上限保护：列号不能大于 TF_IMG_W-1 */
 ...
 *   s_track_half_width = (旧值 * 3 + 新值 + 2) / 4
 ...
 *
 * 单边巡线模式下（g_post_edge_side 为 EDGE_LEFT 或 EDGE_RIGHT），
 * 仅使用一侧的有效边缘，另一侧通过半宽度推算：
 *   - EDGE_LEFT（左侧巡线）：仅用左边缘，右边缘 = 左边缘 + 2*半宽度
 *   - EDGE_RIGHT（右侧巡线）：仅用右边缘，左边缘 = 右边缘 - 2*半宽度
 ...
 *   - EDGE_LEFT：中线 = 左边缘 + 半宽度（靠右侧推算赛道中心）
 *   - EDGE_RIGHT：中线 = 右边缘 - 半宽度（靠左侧推算赛道中心）
 *   - EDGE_BOTH：中线 = (左边缘 + 右边缘) / 2（取两侧边缘中点）
 *
 * @param lb 左边缘列号
 * @param rb 右边缘列号
 * @return 钳位后的中线列号，范围 [0, TF_IMG_W-1]
 */
 ...
 *
 * 搜索策略（按优先级依次尝试）：
 *   1. 检查图像中心列(col=47)附近是否有白色区域，从中心向两侧找边缘
 *   2. 检查 1/4 列(col=23)附近是否有白色区域
 *   3. 检查 3/4 列(col=70)附近是否有白色区域
...
static uint8 find_jidian_at_row(int16 row, int16 *out_lb, int16 *out_rb)
{
    ...
    /* 策略4：以上位置均无白色，从左到右全扫描找左边缘 */
    else
    {
        /* 从图像左端向右扫描，找第一个左边缘 */
        lb = scan_left_edge_right(row, 1, TF_IMG_W - 2);
        /* 若找到左边缘，从图像右端向左扫描找对应的右边缘 */
        if (lb != TF_INVALID)
 ...
 *
 * 从 TF_JIDIAN_ROW(56) 向上搜索到 TF_JIDIAN_MIN_ROW(45)，找到第一个
...
/**
 * @brief 逐行边缘搜索：在指定行中基于上一行的边缘位置搜索新的边缘对
 *
 * 搜索策略（三级回退机制）：
 *   1. 局部搜索：以上一行边缘为中心，左右各 TF_LOCAL_RANGE(10) 像素范围内搜索
 *   2. 反向搜索：若局部未找到，扩大范围反向扫描（防止边缘跳变）
 *   3. 全局搜索：若仍未找到，从图像中心向两侧全扫描
 *   4. 白色段搜索：以上一行中点为参考，查找最近的白色段作为最后手段
 *
 ...
 * @param out_lb  输出参数：当前行左边缘列号
 * @param out_rb  输出参数：当前行右边缘列号
 * @return 1=找到有效边缘对，0=未找到
 */
 ...
 * @brief 巡线融合模块初始化
 *
 * 清零所有边缘/中线/有效行数据，重置误差、基点、阈值等全局状态。
...
/**
 * @brief 计算指定行范围内的加权中线列号
 *
 * 对指定范围内的有效行，按距离加权计算平均中线位置。
 * 权重分配模式：
 *   - top_heavy=0（底部权重高）：越靠近 start_row（底部近相机）权重越大，用于正常巡线
 *   - top_heavy=1（顶部权重高）：越靠近 end_row（顶部远相机）权重越大，用于前瞻弯道预测
 *
 * @param start_row 起始行号（包含，行号大的一端）
 * @param end_row   结束行号（包含，行号小的一端）
 * @param top_heavy 0=底部加权（近处优先），1=顶部加权（远处优先）
...
static int16 calc_weighted_center(int16 start_row, int16 end_row, uint8 top_heavy)
{
    ...
    /* 权重总和 */
    int32 wtotal = 0;
    /* 从 start_row 向 end_row（行号递减）遍历 */
    for (int16 row = start_row; row >= end_row; row--)
    {
        /* 仅处理有效行（边缘对合法的行） */
        if (g_tf.row_valid[row])
        {
            /* 根据 top_heavy 模式计算当前行的权重 */
            int32 w = top_heavy
                ? (int32)(start_row - row + 1)   /* 顶部加权：离顶部（start_row）越近权重越大 */
                : (int32)(row - end_row + 1);    /* 底部加权：离底部（end_row）越近权重越大 */
            /* 累加加权中线列号 */
 ...
 *   3. 计算/使用二值化阈值（Otsu 自适应或固定阈值，由 TF_FIXED_THRESHOLD_ENABLE 控制）
 *   4. 二值化（灰度 -> 黑白）
 *   5. 3x3 邻域降噪（消除孤立噪点）
 *   6. 在底部行搜索基准（基点），失败则判定丢线
 *   7. 从基点向上逐行扫描边缘，同时进行对称分量检测和分支检测
 *   8. 计算加权横向偏差（底部加权，用于正常巡线控制）
 *   9. 计算前瞻误差（顶部加权，用于弯道预测和速度规划）
 *  10. 误差值乘以2缩放到原始坐标系，兼容 PID 参数
 */
void track_fusion_update(void)
{
    ...
    /* ---- 步骤2：压缩 188x120 -> 94x60 ---- */
    compress_image();

#if TF_FIXED_THRESHOLD_ENABLE
    /* ---- 步骤3（固定阈值模式）：使用固定阈值 + 菜单偏移 ---- */
    /* 适合光照稳定的比赛环境，跳过 Otsu 计算以提高帧率 */
    Image_Threshold = clamp_threshold_i16(
        (int16)TF_FIXED_THRESHOLD_VALUE + threshold_bias);
#else
    /* ---- 步骤3（自适应阈值模式）：每隔 TF_OTSU_INTERVAL 帧执行一次 Otsu 计算 ---- */
    /* 递增 Otsu 帧计数器 */
    s_otsu_cnt++;
    /* 计数器达到间隔阈值时，执行 Otsu 计算并重置计数器 */
    ...
    for (int16 row = jrow - 1; row >= end_row; row--)
    {
        ...
        if (search_row_edges(row, prev_lb, prev_rb, &lb, &rb))
        {
            ...
            if (branch_row)
            {
                /* 分支行只用于路口识别，不继续写入普通边线/中线 */
                /* 防止三极管、T口、侧向分支把循迹中线拉歪 */
                break;
            }
            else
            {
    ...
    /* 使用局部作用域限制 la_center 变量的生命周期 */
    {
        /* 计算前瞻区域(行35~20)的加权平均中线（顶部行权重更高） */
 ...
 * 若一侧边缘丢失行数 >= RA_PRE_LOST_THRESH(2)，则设置 g_ra_pre_flag 提前减速。
 *
...
/* 直角/路口标志（1~5），由 detect_intersection() 设置，Pid.c 中的 RA 状态机使用 */
/* 含义：1=右转 2=左转 3=T型左 4=T型右 5=十字 */
uint8 g_ra_flag = 0u;

/* 对称分量标志，1=当前帧检测到对称分量（三极管干扰等） */
uint8 g_sym_component_flag = 0u;

/* 路口检测结果结构体，存储拐点坐标、检测框边界和路口类型 */
IntersectionResult_t g_inter_result;

/* 拐点最大行号（像素行*2），用于 RA 状态机判断何时进入转弯阶段 */
 ...
 * 与真实路口/弯道的不对称特征不同，可以通过形状分析过滤。
 *
 * 判定条件（全部满足才判定为对称分量）：
 *   1. 左右探针行差 <= INTER_SYM_ROW_DELTA(3)（左右对称高度接近）
 *   2. 中心列偏移 <= INTER_SYM_CENTER_DELTA(8)（中心接近图像中心）
 *   3. 左右两侧外延白色段长度 < INTER_SYM_BRANCH_STREAK(18)（非真实分支道路）
 *
...
static uint8 symmetric_component_shape(int16 left_row, int16 right_row, int16 center_col)
{
    ...
    /* 条件1：左右行差不能超过 INTER_SYM_ROW_DELTA(3)，否则不对称 */
    ...
    /* 条件3：若任一侧外延白色段过长(>=18)，说明是真实道路分支，不是对称干扰 */
 ...
 * @brief 从丢失区域向上扫描寻找拐点
 *
 * 从 lost_row 向上扫描，检查左右探针列（ip_left_col / ip_right_col）
 * 是否有白色像素（表示该侧有道路延伸）。
 *
                                    ...
                                    InflectionPoint_t *ip, uint8 *found_side)
{
    ...
    *found_side = 0u;
    ...
    /* 对称分量检查：若左右白色像素构成对称结构，判定为干扰，不作为拐点 */
 ...
 *
 * 在 center_row 为中心、厚度为 INTER_BAND_THICKNESS(3) 行的水平带内，
 * 检查 [col_start, col_end] 范围内白色像素数和最长连续白色段是否达标。
 * 两个条件同时满足才判定有足够白色道路。
 *
 * @param center_row  带中心行号
 * @param col_start   起始列号
 * @param col_end     结束列号
 * @param min_white   最少白色像素数阈值
 * @param min_streak  最长连续白色段阈值
 * @return 1=有足够白色道路，0=无
 */
 ...
 *
 * 在 center_col 为中心、厚度为 INTER_BAND_THICKNESS(3) 列的垂直带内，
 * 检查 [row_start, row_end] 范围内白色像素数和最长连续白色段是否达标。
 * 两个条件同时满足才判定有足够白色道路。
 *
 * @param center_col  带中心列号
 ...
 * @param side       1=检查右侧，2=检查左侧
 * @return 1=有足够长的分支道路，0=无
 */
static uint8 inter_side_branch_has_road(int16 row,
                                        int16 center_col,
                                        uint8 side)
{
    ...
    if (side == 1u)
    {
        /* 右侧：从 center_col + 最小赛道宽度 到 图像右边界 */
        col_start = center_col + branch_gap;
        col_end = TF_IMG_W - 1;
    }
    else
    {
        /* 左侧：从 图像左边界 到 center_col - 最小赛道宽度 */
        col_start = 0;
 ...
 * @brief 清除路口检测结果结构体
 *
 * 将 g_inter_result 的所有字段清零，包括左右拐点、检测框边界和路口类型。
...
/**
 * @brief int16 钳位函数：将值限制在 [min_v, max_v] 范围内
 *
 ...
 *
 * 估算策略（三级回退）：
 *   1. 从 cy 行向上下各搜索 8 行，找到第一个有效边缘对，用其宽度
 *   2. 若都不行，回退使用基点(left_jidian, right_jidian)边缘对宽度
 *   3. 兜底使用 BOX_WIDTH/INTER_BOX_WIDTH_SCALE 默认值
 *
 * 赛道宽度估算用于动态调整检测框的大小。
 *
 * @param cy 中心行号（拐点行）
 ...
 * @param cy     框中心行号
 * @param top    输出参数：框顶行号
 * @param bottom 输出参数：框底行号
 * @param left   输出参数：框左列号
 * @param right  输出参数：框右列号
 */
static void build_inter_box(int16 cx, int16 cy,
                            int16 *top, int16 *bottom,
                            int16 *left, int16 *right)
...
void detect_intersection(void)
{
    ...
    if (s_inter_lock_cnt > 0u)
    {
        ...
        else
        {
            /* RA 仍在执行，递增锁定计数器 */
            s_inter_lock_cnt++;
            /* 锁定超时保护：最多锁定 INTER_MAX_LOCK_FRAMES(300) 帧（约3.3秒） */
    ...
    /* 不满足检测条件：无丢线 且 无预检测 且 无高行丢失 -> 清除并返回 */
    if (g_tf.line_lost == 0u &&
        g_ra_pre_flag == 0u &&
        s_first_miss_row < (int16)INTER_MIN_MISS_ROW &&
        !probe_trigger &&
    ...
    if (voted_type != 0u)
    {
        /* 设置路口类型标志（1~5），Pid.c 中的 RA 状态机据此执行转弯 */
