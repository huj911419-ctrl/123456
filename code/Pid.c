...
static float cascade_steer_calc(int16 pos_err, float kp_scale, float kd_scale, float ff_scale, float slew_max) /* 串级PID�� */
{
    ...
    /* IMU零漂慢速跟踪: 只在转速低时学习,防止转弯角速度被当成零漂 */
    ...
    /* 计算�标��度 = 位置�?+ 趋势前� + 前瞻前� */
    float yaw_target = CAS_POS_KP  * kp_scale * err * p_scale          /* 位置项：��×比例 */
                     + CAS_TREND_KD * aux_scale * (float)g_tf.error_trend /* 趋势前� */
                     + CAS_LA_K    * aux_scale * (float)g_tf.lookahead_error; /* 前瞻前� */
...
static int16 calc_quality_speed_pct(void) /* 视�质量降速百分比 */
{
    ...
    if (g_tf.valid_row_count < SPEED_QUALITY_GOOD_ROWS) /* 有效行不�?35) */
    {
        ...
        {
            /* 有效行极少，�大降�?*/
            row_pct = SPEED_VISION_BAD_PCT; /* �大降�?40%) */
        }
...
static int16 plan_base_speed(int16 target, int16 pos_err_abs, uint8 pre_slow_active) /* 速度规划 */
{
    ...
    if (g_tf.line_lost != 0u)
    {
        s_straight_cnt = 0u;                /* 直道计数清零 */
        s_straight_hold = 0u;
        speed_dbg_vq_pct = SPEED_LINE_LOST_PCT; /* 记录降�百分比(25%) */
        target = (int16)((int32)target * SPEED_LINE_LOST_PCT / 100); /* 降�到25% */
    ...
    if (g_tf.valid_row_count < SPEED_VISION_BAD_VALID_ROWS) /* 有效行极�?�?8) */
    {
        s_straight_cnt = 0u;                /* 直道计数清零 */
        s_straight_hold = 0u;
        speed_dbg_vq_pct = SPEED_VISION_BAD_PCT; /* 记录降�百分比(40%) */
        target = (int16)((int32)target * SPEED_VISION_BAD_PCT / 100); /* 降�到40% */
