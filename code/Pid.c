...
/* 速度规划原因编号，用于TFT调试显示（1=RA 2=丢线 3=视觉差 4=对称加速 5=前瞻 6=直道加速 7=质量 8=正常） */
...
static const IntersectionRule user_rules[] = {
    ...
    RULE_RA_EDGE(3u, 1u, EDGE_LEFT, SINGLE_EDGE_POST_TURN_MS), /* 第3个flag=1（右直角）：结束后靠左单边 */
...
static int16 speed_ramp_apply_reason(int16 target, uint8 reason) /* 带原因的斜坡 */
{
    ...
    /* 直道加速原因(4/6)使用更快的上升步长 */
    if (reason == 4u || reason == 6u)       /* 直道加速原因 */
...
static int16 plan_base_speed(int16 target, int16 pos_err_abs, uint8 pre_slow_active) /* 速度规划 */
{
    ...
    if (sym_straight_speed_candidate(pos_err_abs)) /* 是对称组件直道候选 */
    {
        ...
        target = (int16)((int32)target * SPEED_STRAIGHT_BOOST_PCT / 100); /* 加速到105% */
...
static void normal_pid_step(int16 pos_err, int16 pos_err_abs) /* 正常PID控制 */
{
    ...
    /* 自适应速度目标 */
    float target_speed = (float)calc_adapted_speed(base_speed, speed_err); /* 误差自适应速度 */
