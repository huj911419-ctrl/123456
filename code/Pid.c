...
static const IntersectionRule user_rules[] = {
    ...
    RULE_RA_EDGE(2u, 1u, EDGE_LEFT, SINGLE_EDGE_UNTIL_NEXT_TURN), /* 第2个flag=1（右直角）：结束后靠左单边 */
...
static uint8 ra_turn_row_for_speed(void)
{
    ...
    if (s_ra_orig_flag < 3u)
    {
        ...
        {
            float fast_t = range_ratio_i16(speed_ref,
                                           RA_FAST_SPEED_START,
                                           RA_FAST_TURN_ROW_ADVANCE_SPEED_END);
            float extra = (float)((int16)RA_FAST_TURN_ROW_ADVANCE_MAX -
                                  (int16)RA_FAST_TURN_ROW_ADVANCE) * fast_t;
            row -= (int16)((float)RA_FAST_TURN_ROW_ADVANCE + extra + 0.5f);
        }
...
static float ra_pre_turn_steer_ff(void)
{
    ...
    if (dir == 0u || s_turn_shield_frames != 0u ||
...
static uint8 ra_next_flag_ready(void)
{
    return (g_ra_flag != 0u &&
            ra_accept_near_flag((uint8)g_ra_flag)) ? 1u : 0u;
}

static uint8 ra_next_pre_ready(void)
{
    return (g_ra_pre_flag != 0u &&
            (g_ra_pre_dir == 1u || g_ra_pre_dir == 2u) &&
            route_next_flag_is((uint8)g_ra_pre_dir)) ? 1u : 0u;
}

static uint8 ra_post_gap_holds(void)
{
    if (s_ra_post_turn_gap_cnt == 0u)
        return 0u;

    if (ra_next_flag_ready() || ra_next_pre_ready())
    {
        s_ra_post_turn_gap_cnt = 0u;
        return 0u;
    }
...
static void turn_shield_start(void)         /* 启动转弯屏蔽 */
{
    ...
    ra_clear_all_flags();                   /* 清除当前RA/pre标志 */
...
static void turn_shield_update(void)        /* 转弯屏蔽帧更新 */
{
    ...
    /* 屏蔽期间检测到下一个可接受弯 → 提前解除 */
    if (s_turn_shield_frames >= TURN_SHIELD_NEAR_ALLOW_FRAMES &&
        (ra_next_flag_ready() || ra_next_pre_ready()))
    {
        turn_shield_reset();                /* 复位转弯屏蔽 */
        return;                             /* 提前解除 */
    }

    if (g_ra_flag != 0u)                    /* 未被接受的正式flag */
    {
        g_ra_flag = 0u;                     /* 清除RA flag */
    }

    if (g_ra_pre_flag != 0u)                /* 非路线预期预判仍清掉 */
        ra_clear_pre_flags();
...
static void ra_finish_ex(uint8 keep_flag, uint8 use_shield) /* RA结束扩展 */
{
    ...
    else if (s_ra_post_edge_side == EDGE_LEFT || s_ra_post_edge_side == EDGE_RIGHT) /* 规则指定了具体方向 */
    {
        /* 规则指定了具体方向 */
...
static int16 calc_lookahead_speed_pct(int16 pos_err_abs) /* 前瞻/趋势降速百分比 */
{
    ...
    int16 la_pct = calc_signal_speed_pct(la_abs,          /* 前瞻信号 */
                                         SPEED_LOOKAHEAD_SLOW_T1, /* 前瞻降速下限 */
                                         SPEED_LOOKAHEAD_SLOW_T2, /* 上限28 */
                                         SPEED_LOOKAHEAD_MIN_PCT); /* 前瞻最低速度百分比 */
    /* 趋势降速百分比 */
    int16 trend_pct = calc_signal_speed_pct(trend_abs,     /* 趋势信号 */
                                            SPEED_TREND_SLOW_T1,   /* 趋势降速下限 */
                                            SPEED_TREND_SLOW_T2,   /* 上限24 */
                                            SPEED_TREND_MIN_PCT);  /* 趋势最低速度百分比 */
    /* 取较小值（降速更多） */
                                ...
                                float yaw_progress_rate)
{
    ...
    if (yaw_progress >= hard_yaw_target - RA_HARD_COAST_REMAIN_DEG &&
        yaw_progress_rate >= RA_HARD_COAST_YAW_RATE)
    {
        return 1u;
    }

    return (yaw_progress >= hard_yaw_target) ? 1u : 0u;
}
...
static int16 plan_base_speed(int16 target, int16 pos_err_abs, uint8 pre_slow_active) /* 速度规划 */
{
...

    {
        ...
        if (straight_speed_candidate(pos_err_abs)) /* 是直道候选 */
        {
            ...
            if (s_straight_cnt >= SPEED_STRAIGHT_CONFIRM_FRAMES) /* 连续确认帧达标 */
            {
                s_straight_active = 1u;         /* 激活直道模式 */
                target = (int16)((int32)target * SPEED_STRAIGHT_BOOST_PCT / 100); /* 直道加速 */
                reason = 6u;                    /* 原因=直道加速 */
...
static uint8 ra_handle_recover_phase(RaResult *r)
{
    ...
    s_ra_recover_good_cnt =
        (s_ra_recover_cnt > 255u) ? 255u : (uint8)s_ra_recover_cnt;
    ra_clear_all_flags();
...
static uint8 ra_step_wait_slow_approach(RaResult *r)
{
    if (s_ra_phase == RA_PH_WAIT)
    {
        ...
        if (s_ra_ip_row >= (uint8)ra_slow_row || wait_timeout)
        {
            s_ra_phase = RA_PH_SLOW;
            s_ra_phase_cnt = 0u;
    ...
    else if (s_ra_phase == RA_PH_SLOW)
    {
        ...
        uint8 slow_timeout = (s_ra_phase_cnt >= slow_limit) ? 1u : 0u;
        ...
        if (s_ra_ip_row >= turn_row || slow_timeout)
        {
...
static RaResult ra_state_machine_step(int16 pos_err_abs) /* RA状态机主逻辑 */
{
    ...
    if (s_ra_state == RA_ST_NONE && ra_post_gap_holds())
    {
        ra_debug_update();
        return r;
    }
...
static void normal_pid_step(int16 pos_err, int16 pos_err_abs) /* 正常PID控制 */
{
    ...
    if (s_ra_state == RA_ST_ACTIVE && s_ra_phase == RA_PH_RECOVER) /* RECOVER阶段 */
    {
        steer *= recover_steer_scale();      /* 转向渐入，避免出弯抖动 */
    }
