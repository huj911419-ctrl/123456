...
#define IMU_YAW_GAIN              1.15f    /* yaw gain for conservative turn exit */
...
void imu_update(void)
{
    ...
    rate = (imu_read_z_rate_dps() - s_gyro_z_offset) * IMU_YAW_GAIN;  /* 读取角速度并减去校准得到的零偏值 */
    ...
    yaw_angle += rate * IMU_UPDATE_DT_SEC;                   /* 角速度乘以时间步长（0.005s），累加到偏航角 */
